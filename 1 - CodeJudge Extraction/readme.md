...
```
python3 add_location_to_evals.py --apps [PATH] --input [PATH] --output [PATH]
```
2. Run evaluate_code_solutions script. This script does multi-threading to first of all run the original APPS solution on its own code and then run all of the test cases on "code" field from `X_with_location.json`. Label and evaluation details are added and a. new `X_with_locations_with_evaluations.json` file is created. Some sols will be marked as "NA"
```
python3 evaluate_code_solutions.py --input [PATH] --output [PATH]
```
3. Add additional metrics
```
python3 add_additional_metrics_to_evals.py --apps [PATH] --input [PATH] --output [PATH] --api-key [OpenAI Key]
```
